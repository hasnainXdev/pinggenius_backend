...
# Create a test client with MongoDB connection patched
@pytest.fixture
def client():
    with patch('database.mongo.mongodb') as mock_mongodb:
        # Mock the MongoDB connection
        mock_mongodb.connect = MagicMock()
        mock_mongodb.disconnect = MagicMock()
        mock_db = MagicMock()
...
def test_generate_outreach_sequence_success(mock_mongodb):
    ...
    with patch('api.outreach.router.sequence_service') as mock_sequence_service:
        ...
        mock_sequence.created_at = datetime.now()
        mock_sequence.updated_at = datetime.now()
...
def test_get_outreach_sequence_success(mock_mongodb):
    ...
    mock_sequences_collection = MagicMock()
    now = datetime.now()
    mock_sequences_collection.find_one.return_value = {
